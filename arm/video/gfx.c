...
void SRAM_TEXT draw_string(gfx_screen_t screen, const char* str, int x, int y, u32 color)
{
	if(screen == GFX_ALL) {
		for(int i = 0; i < GFX_ALL; i++)
...
void SRAM_TEXT sram_print(const char* msg) {
	...
	draw_string(screen, msg, fbs[screen].current_x, fbs[screen].current_y, GREEN);
